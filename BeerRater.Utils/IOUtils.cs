...
namespace BeerRater.Utils
{
    ...
    public static class IOUtils
    {
        ...
        {
            if (string.Compare(sourceFolder, destFolder, true) == 0) return;
        ...
        public static void MoveFile(string Source, string Destination, bool Overwrite)
        {
            if (Source == Destination)
                throw new ArgumentException(string.Format(
                    "Source and destination are the same. Cannot move file [{0}].",
                    Source));

            if (File.Exists(Source))
            {
                if (IsFileReadOnly(Source)) File.SetAttributes(Source, FileAttributes.Normal); // Remove Read-only tag
            ...
            {
                throw new ArgumentException(string.Format(
                    "Source file does not exist. Cannot move file [{0}] to [{1}].",
                    Source, Destination));
            }
        }
